...
public class Percolation {
   ...
   public Percolation(int N) {
       ...
       nodes2 = new WeightedQuickUnionUF(size*size);
   ...
   public void open(int i, int j) {
       ...
       // four different cases, if it is the first line then it is open
       // if it is in the middle, you have to check if it is connected to the top
       ...
       if (i > 1){
           if (openness[xyTo1D(i-1, j)]) {
               nodes.union(xyTo1D(i, j), xyTo1D(i-1, j));
               nodes2.union(xyTo1D(i, j), xyTo1D(i-1, j));//check the connection with the top
           }
       }
       if (i < size) {
           if (openness[xyTo1D(i+1, j)]) {
               nodes.union(xyTo1D(i, j), xyTo1D(i+1, j)); //check the connection with the bottom
               nodes2.union(xyTo1D(i, j), xyTo1D(i+1, j));
           }
       }
       if (j > 1) {
           if (openness[xyTo1D(i, j-1)]) {
               nodes.union(xyTo1D(i, j), xyTo1D(i, j-1)); //check the connection with the left
               nodes2.union(xyTo1D(i, j), xyTo1D(i, j-1));
           }
       }
       if (j < size){
           if (openness[xyTo1D(i, j+1)]) {
               nodes.union(xyTo1D(i, j), xyTo1D(i, j+1)); //check the connection with the right
               nodes2.union(xyTo1D(i, j), xyTo1D(i, j+1));
           }
       }
   }
